...
public class P3_InsertionSort {
    static void insertionSort(int[] arr,int n){
        for(int i=0;i<n;i++){
            ...
            while(j>0 && arr[j-1]>arr[j])   //j>0 because when element arr[j] comes till 1st pos (becoz j--) then it will check with arr[j-1] to swap. So no need to go in 0-ind pos when it can be swapped from 1-ind pos
            {
                int temp = arr[j-1];
                arr[j-1] = arr[j];
                arr[j] = temp;
