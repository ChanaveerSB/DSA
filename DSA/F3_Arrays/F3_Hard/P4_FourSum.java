...
public class P4_FourSum {
    ...
    public static List<List<Integer>> fourSumOptimal(int[] nums,int target) {
        ...
        Arrays.sort(nums);
        for(int i=0;i<n;i++){
            if(i!=0 && nums[i]==nums[i-1]) continue;    //skips the j k l parts and increment i
            for(int j=i+1;j<n;j++){
                ...
                while(k<l){
                    ...
                    if(sum<target) k++;
                        ...
                        k++;
                        l--;
                        // k++ and l-- should happen till they should point to unique ele than current sum used ele
