...
public class P3_ThreeSum {  //target = 0
    ...
    public static List<List<Integer>> threeSumPointer(int[] nums) {
        ...
        Arrays.sort(nums);
        for (int i = 0; i < n; i++) {
            // i should'nt increment when in first index so i>0 it can increment only after
            // j>k (video : 32:08) then compare with next ele to move next unique element
            // from previous
            if (i > 0 && nums[i] == nums[i - 1])
                continue; // continue becoz for will however increments
            int j = i + 1;
            int k = n - 1;
            while (j < k) {
